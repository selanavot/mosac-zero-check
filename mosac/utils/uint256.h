...
inline constexpr uint256::uint256(const uint256_pod& v)
    : lo_(v.lo), hi_(v.hi) {}
inline uint256::uint256(uint128_t bottom) : hi_(0) {
  auto [high64, low64] = yacl::DecomposeUInt128(bottom);
  lo_ = absl::MakeUint128(high64, low64);
}
...
inline uint256::operator uint128_t() const {
  return yacl::MakeUint128(absl::Uint128High64(lo_), absl::Uint128Low64(lo_));
}
...
namespace std {
template <>
class numeric_limits<mosac::uint256> {
 public:
  ...
  static constexpr mosac::uint256(min)() { return 0; }
  static constexpr mosac::uint256 lowest() { return 0; }
  static constexpr mosac::uint256(max)() { return mosac::Uint256Max(); }
  static constexpr mosac::uint256 epsilon() { return 0; }
  static constexpr mosac::uint256 round_error() { return 0; }
  static constexpr mosac::uint256 infinity() { return 0; }
  static constexpr mosac::uint256 quiet_NaN() { return 0; }
  static constexpr mosac::uint256 signaling_NaN() { return 0; }
  static constexpr mosac::uint256 denorm_min() { return 0; }
};
}  // namespace std

template <>
struct fmt::formatter<mosac::uint256> {
  constexpr auto parse(fmt::format_parse_context& ctx) { return ctx.begin(); }

  template <typename FormatContext>
  auto format(const mosac::uint256& number, FormatContext& ctx) {
    return fmt::format_to(ctx.out(), "{}", std::string(number));
  }
};
